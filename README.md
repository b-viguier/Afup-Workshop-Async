...
You only need to clone this repository, and go in corresponding directory.
```bash
...
The workshop is divided in several steps.
For each step:
...
* Launch tests (regularly if needed) to check if your code has expected behavior,
  using command `./vendor/bin/phpunit`.
* If you need some clues, you can use `git rebase step-<n>-solution` to fill `src_help` folder with pre-written classes.
  Just copy what you need in you own `src` folder.

Of course, never hesitate to ask questions! 😉

## Part A

In this part, we will create all the tools to create our crawler with *asynchronous design*… **but synchronously**!
...
Then we create our initial `EventLoop`,
the `wait` method is a **synchronous** way to retrieve the resolved value of a promise.
If the promise is rejected, this method must throw corresponding exception.
This method should be called only once on the *global* promise of your program,
...
to create a promise that will be resolved when all its children promises are resolved.
But if one child promise fails, it's enough to fail the parent promise. 
...
Feel free to use CLI tools you are comfortable with, there is no predefined structure here, no tests to execute.
...
https://b-viguier.github.io/Afup-Workshop-Async/api/text.json
* Each sentence is accessible according to the given relative path: 
https://b-viguier.github.io/Afup-Workshop-Async/api/sentence/a53441.json for example.  
* In the same way, you can retrieve details about words composing a sentence:
https://b-viguier.github.io/Afup-Workshop-Async/api/word/5e2f8e.json
* To finish, the value of each letter can be requested:
https://b-viguier.github.io/Afup-Workshop-Async/api/letter/7fc562.json
...
⚠️ If you code it in *bare Php*n do not forget to `include __DIR__.'/../vendor/autoload.php';`
in order to enable [classes autoloading](https://secure.php.net/manual/en/language.oop5.autoload.php).
💡 Think about a way to follow the progress of your program, and possible errors encountered.
...
Don't try to improve performance with the built-in Php web server,
it can run only one request at a time.
...
it will be a good way to see how it deals with them.
* [ReactPhp](https://reactphp.org/) use [Promises/A](https://reactphp.org/promise/) implementations,
it will be interesting to convert promises with generators.
...
by requiring a *token* to perform a request.
